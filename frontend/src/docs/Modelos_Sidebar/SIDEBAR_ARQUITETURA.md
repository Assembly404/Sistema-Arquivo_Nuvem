...
> **Status:** v1.2 — as **Fases 1 e 2 estão implementadas em código** (ver secção 6), incluindo as **correções de UI da revisão v1.2** (scroll da navegação, popovers portados e menu de conta em modo colapsado — ver 6.2 #11–#21); a **Fase 3** (estados de navegação por permissão) permanece em **planejamento/decisão**. Toda a documentação de decisão e de implementação vive neste documento — os arquivos de código **não recebem comentários**.
>
...
| `frontend/src/components/header/sidebar.jsx` | implementado (v1.2) | `SideBar` (default) + `SidebarItem` (export nomeado), estados de 2.5, conflitos C2/C3/C4/C6/C7; scroll do `<nav>`, popovers portados (variantes `menu`/`label`/`reason`), sincronização de estado por rota, `inert` e bloqueio de scroll do `body` no mobile |
| `frontend/src/components/header/sidebar.css` | implementado (v1.2) | `.sidebar`/`.sidebar-item` de 4.6 + scroll interno e *fade* de indicador, popovers `position: fixed`, overlay, rodapé, menu de conta (`.sidebar-menu*`) e animações `dd-in`/`dd-in-up` de 4.7 |
| `frontend/src/pages/dashboard/DashboardLayout.jsx` | **novo** | D2: `<SideBar/>` + `<main>` com `<Outlet/>`; preferência `collapsed` em `localStorage` |
| `frontend/src/pages/dashboard/DashboardLayout.css` | **novo** (v1.2) | offset do conteúdo via `--sidebar-width` / `--sidebar-width-collapsed` + shell `.dashboard-page` |
| `frontend/src/App.css` | alterado (v1.2) | tokens de layout `--sidebar-width` / `--sidebar-width-collapsed` (§1.12) e nota de camadas de z-index (§1.9) |
...
|---|---|---|
| 1 | C3 (flyout em modo colapsado) | **Revisado em v1.2 (ver #11):** com o scroll interno do `<nav>`, o flyout não pode ser `position: absolute` — era recortado pelo scroll container. Passou a popover portado para `document.body`, `position: fixed`, `z-index: var(--z-navbar)` e posicionamento por JS (`getBoundingClientRect` + *clamp* vertical), exatamente como C3 previa. |
...
| 7 | Estado desabilitado | `SidebarItem` renderiza `aria-disabled="true"`/`disabled` + `aria-label` com o motivo, lendo `item.disabled` / `item.disabledReason` do nó — mas **nenhum nó é desabilitado até a Fase 3**, que é quem decide permissão (6.4.1). Itens desabilitados saem como `<span>` (bloqueio real de clique e de teclado, não apenas visual). **v1.2:** o motivo deixou de estar só no `title` nativo e passou a aparecer num popover (ver #14). |
| 8 | Classes do menu de conta | **Revisado em v1.2:** a sidebar deixou de consumir `.dropdown-menu`/`.dropdown-item` (spec 4.7) e passou a usar `.sidebar-menu`/`.sidebar-menu__item` com os mesmos valores — evita colisão nominal com `components/dropdown/dropdown.jsx` (que usa `.dropdown-*` para um select de formulário, com `dropdown.css` vazio). Mantidos `display: flex; gap`, agora com `white-space: nowrap` + ellipsis no label. |
...
| 11 | **P0-A · scroll do `<nav>`** | `.sidebar-nav-wrap` (`display: flex; flex: 1 1 auto; min-height: 0`) + `.sidebar-nav` (`min-height: 0; overflow-y: auto; overscroll-behavior: contain`, scrollbar oculta) e `flex-shrink: 0` no cabeçalho e no rodapé. Sem `min-height: 0`, o `flex: 1` não encolhia abaixo do conteúdo: com submenus abertos o rodapé de conta (com "Terminar sessão") era empurrado para fora da viewport e os últimos itens ficavam inalcançáveis. O `padding` de `--space-1` com `margin` negativo equivalente preserva a largura dos itens e dá gutter ao `outline` de foco. Indicador de scroll = *fade* `.sidebar-nav-wrap[data-scroll-top/bottom="true"]`, calculado por `ResizeObserver` (nav + lista) e evento `resize` — sem `setState` síncrono dentro de `useEffect`, por causa da regra `react-hooks/set-state-in-effect` do ESLint v7. |
| 12 | **Camadas do popover portado (decisão Z1)** | Como o popover vive em `document.body`, `--z-dropdown` (100) ficaria abaixo da sidebar (`--z-sticky`: 500). Usa-se `--z-navbar` (1000) e mantém-se a ordem dos tokens em `App.css`; a semântica ficou registada em §1.9 da Identidade Visual. `.sidebar-overlay` mantém `--z-dropdown`, que continua abaixo da sidebar (100 < 500). |
| 13 | **P0-B · menu de conta em modo colapsado** | A largura deixou de ser `left: 0; right: 0` (no rail dava 56px e partia "Terminar sessão" em várias linhas) e passou a ser fixa em `calc(var(--sidebar-width) - (var(--space-4) * 2))` = 228px — a largura interna do modo expandido — nos dois modos. No colapsado ancora à direita do rail (`left: calc(100% + var(--space-2)); bottom: 0`), como os popovers dos mockups 2 e 3, sem tapar os ícones. Entrada com `dd-in-up` (painel abre para cima). |
| 14 | **Um único popover com três variantes** | O tooltip em CSS (`::after` + `attr(data-label)`) e os `title` nativos foram substituídos por um popover com variantes `menu` (submenu), `label` (rótulo do item) e `reason` (motivo do item desabilitado), resolvidas pelo `reason`/`children` do nó. O motivo do item desabilitado passa a ser visível em hover/focus (o `pointer-events: none` de 4.6 impede o `title` nativo de abrir) e audível via `aria-label` no formato `"<label> — <motivo>"` — T3 fica verificável. |
| 15 | **Reabertura do ramo ativo** | `openSubmenus` passa a ser sincronizado na mudança de rota com os ancestrais do item ativo, corrigindo o caso em que se navega por um popover em modo colapsado e o agrupador continuava fechado ao expandir. Alternar um agrupador em modo colapsado deixou de alterar estado invisível (`handleToggleSubmenu` sai cedo quando `isCollapsed`). |
| 16 | **Reset de estado sem efeitos** | O reset de `isMobileOpen`/`accountMenuOpen`/`popover` (por rota ou por tipo de dispositivo) e o fecho do popover (por colapso) usam **ajuste de estado durante o render** (comparação com `renderedNav`/`renderedCollapse`), o padrão documentado do React para "reiniciar estado quando uma prop muda". A alternativa com `useEffect` foi rejeitada pela regra `react-hooks/set-state-in-effect` (v7) e por provocar renders em cascata. |
| 17 | **Mobile · `inert` e bloqueio de scroll** | Abaixo de 992px, com o drawer fechado, o `<aside>` recebe `inert` (os links off-canvas deixam de ser tabuláveis); com o drawer aberto, `document.body.style.overflow = "hidden"` é aplicado e restaurado na limpeza do efeito. O acionador flutuante passou a usar a superfície glass de 4.5 (`--color-glass` + `backdrop-filter`), deixando de ser um botão opaco sobre o conteúdo ao rolar. |
| 18 | **Affordance do rodapé de conta** | Acrescentado `FiChevronDown` (`.sidebar-account__chevron`) com rotação em `[aria-expanded="true"]` e oculto no modo colapsado, alinhando o rodapé ao que os mockups 1 e 3 mostram (afordância à direita do cartão). |
| 19 | **Marca em modo colapsado** | O cabeçalho colapsado passou a coluna (marca **+** botão), como nos mockups 1 e 3; antes a marca era removida com `display: none`. |
| 20 | **Contraste do item destrutivo** | Texto em `--color-danger` (≈10:1) e ícone em `--color-danger-action` (uso não-textual: ≈4:1 ≥ 3:1); hover `color-mix(in srgb, var(--color-danger-bg) 25%, var(--color-surface))` (≈8:1). A regra anterior – texto a `--color-danger-action` (≈4:1, e ≈2.6:1 sobre `--color-danger-bg`) – violava o item AA do checklist da Identidade Visual; a correção está refletida em §4.7. |
| 21 | **Larguras centralizadas** | `--sidebar-width` (260px) e `--sidebar-width-collapsed` (72px) em `App.css` (§1.12) eliminam os literais `260px` (3×) e `calc(var(--space-16) + var(--space-2))` (2×) que existiam em `sidebar.css` e `DashboardLayout.css`, agora ambos ligados aos tokens. |

### 6.3 Verificação executada

| Verificação | Resultado |
|---|---|
| `npm run build` (v1.1) | ✅ build de produção sem erros |
...
| `npm run lint` (v1.2) | ✅ 0 erros em `sidebar.jsx`, `sidebar.css`, `DashboardLayout.css` e `App.css`; mantêm-se apenas os 5 erros pré-existentes (`input.jsx`, `toast.jsx`, `Home.jsx`) |
| `npm run build` (v1.2) | ✅ build de produção sem erros (`index.css` 26.33 kB, `index.js` 435.46 kB) |
| Render SSR descartável (v1.2) | ✅ **20/20** checagens: `.sidebar-nav-wrap` com `data-scroll-top`/`data-scroll-bottom`, `<nav class="sidebar-nav">` presente, submenu do item ativo aberto, `aria-current` no filho, `sidebar-item--parent-active` no pai, `aria-expanded="true"` no pai, menu de conta fechado por omissão, chevron do rodapé presente, modo colapsado aplicado sem submenu inline e sem popover no DOM, `<aside>` com `inert` no mobile fechado, trigger só com o drawer fechado, `SidebarItem` desabilitado com o motivo no `aria-label` e **sem** `title` nativo, e ausência de `aria-controls` pendente |
| Validação manual pendente (v1.2) | ⏳ roteiro de 6.4.7 — cenários de altura reduzida, popover *clamp*/*flip*, menu de conta nos dois modos, teclado e mobile |

Itens do checklist §5 já cobertos pelas Fases 1–2: menu 1:1 com o inventário; `activePath` derivado de `useLocation()` (sem estado duplicado); item pai reflete o filho ativo; flyout na sidebar colapsada; off-canvas abaixo de 992px; uso exclusivo de `var(--*)`; estados normal/hover/active/disabled/focus-visible presentes; alvo ≥ 44px; persistência de `collapsed` em `localStorage`; rodapé de conta fora do `<nav>`. Itens acrescentados na v1.2: indicador visível na área com scroll interno; marcador de separação do rodapé mantido com o `<nav>` a rolar; popovers sempre dentro da viewport (*clamp*/*flip*) e fechados por `Esc`/clique fora/scroll; contraste AA no item destrutivo; `inert` + bloqueio de scroll do `body` no mobile; nenhum `title` nativo como única afordância de estado. Continuam **abertos** (dependem da Fase 3): T2, T3, T8, T9, `loadingPermissions` e o bloqueio de rotas por permissão.
...
4. ~~**Scroll interno da sidebar**~~ **Resolvido em v1.2** (ver 6.2 #11/#12): o `<nav>` tem scroll próprio com *fade* de indicador e o flyout migrou para `position: fixed` portado para `document.body`, como C3 previa. Falta apenas confirmar em viewports baixas no roteiro manual (6.4.7).
...
7. **Roteiro manual de verificação:** `npm run dev` → `/login` → entrar → `/dashboard`. Fases 1–2: navegação nos itens e submenus; item pai destacado ao entrar em `Arquivos → Compartilhados comigo`; colapso pelo botão do topo da sidebar (a preferência fica em `localStorage`, chave `sidebar:collapsed`); janela abaixo de 992px (sidebar off-canvas com overlay, `Esc`/clique no backdrop fecham); menu de conta no rodapé ("Perfil" e "Terminar sessão", que limpa a sessão e volta à área pública). **v1.2:** (a) com `Arquivos` **e** `Administração` abertos, reduzir a janela para ~500/600/700px de altura → o `<nav>` rola, o rodapé de conta continua visível e "Configurações" é alcançável, com o *fade* no topo/base; (b) em modo colapsado, passar o rato/pôr o foco no último grupo (`Administração`) e nos itens finais → o popover nunca sai da viewport e fecha ~150ms depois de sair; (c) colapsar, clicar no cartão de conta → "Perfil" e "Terminar sessão" numa única linha, com ícone alinhado (mesma forma do modo expandido); (d) navegar por um popover (`Arquivos → Lixeira`), expandir a sidebar → o agrupador ativo reabre; (e) teclado: `Tab` até ao agrupador em modo colapsado abre o popover, `Tab` entra nos filhos, `Esc` fecha e devolve o foco; (f) mobile: com o drawer fechado o `Tab` **não** entra nos links da sidebar, com o drawer aberto o fundo não rola, e o acionador flutuante é glass; (g) verificar `prefers-reduced-motion` (as entradas `dd-in`/`dd-in-up` ficam instantâneas).

---

*Documento v1.2 — consolidado a partir de `IDENTIDADE_VISUAL.md` (secções 1.9, 1.12, 3.6, 4.6, 4.7, 4.9, 4.10, 5.1, 5.4, 5.5, 6), `RegrasDeTecnologias.md`, dos scripts de schema (`001_create_usuario.sql` … `006_create_armazenamento.sql`), do código atual (`AuthController.java`, `LoginResponse.java`, `User.java`, `AuthProvider.jsx`, `Login.jsx`, `App.jsx`, `sidebar.jsx`, `spinner.jsx`, `package.json`, `vite.config.js`) e dos 3 mockups desta pasta. É a referência normativa da implementação do componente `SideBar` — as **Fases 1 e 2 já estão em código** (secção 6), com o adendo de correções v1.2 (#11–#21), e a Fase 3 permanece em decisão. Alterações a este documento devem manter a coerência com o `IDENTIDADE_VISUAL.md`.*
